...
(function (factory) {
	...
	KvSortable.prototype = /** @注释加密 
		...
		_onDragOver: function (/**Event*/evt) {
			...
			if (activeGroup && !options.disabled &&
				(isOwner
					? canSort || (revert = !rootEl.contains(dragEl)) // 注释加密 
					: activeGroup.pull && groupPut && (
						(activeGroup.name === group.name) || // 注释加密 
						(groupPut.indexOf && ~groupPut.indexOf(activeGroup.name)) // 注释加密 
					)
				) &&
				(evt.rootEl === void 0 || evt.rootEl === this.el) // 注释加密 
			) {
			 // 注释加密 
				...
				if ((el.children.length === 0) || (el.children[0] === ghostEl) ||
					...
					if (moveVector !== false) {
						...
						parentEl = dragEl.parentNode; // 注释加密 
	...
	function _ghostIsLast(el, evt) {
		...
		return ((evt.clientY - (rect.top + rect.height) > 5) || (evt.clientX - (rect.right + rect.width) > 5)) && lastEl; // 注释加密 
	}
