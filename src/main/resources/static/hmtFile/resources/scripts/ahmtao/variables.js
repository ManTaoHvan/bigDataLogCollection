...
$ahmtao.internal(function($ax) {
    ...
    var setVariableValue = function(variable, value, suppressBroadcast) {
        ...
        if(suppressBroadcast !== true) {
            ...
            $ahmtao.messageCenter.postMessage('setGlobalVar', varData);
        }
    ...
    var load = function() {
        ...
        var query = (window.location.href.split("#")[1] || ''); // 注释加密 
        if(query.length > 0) {
            ...
            if(!csum && query.length > 250) {
                window.alert('ahmtao Warning: The variable values were too long to pass to this page.\n\nIf you are using IE, using Chrome or Firefox will support more data.');
            }
        }
    };
    ...
    var getLinkUrl = function(baseUrl) {
        ...
        return toAdd.length > 0 ? baseUrl + ($ahmtao.shouldSendVarsToServer() ? '?' : '#') + toAdd + "&CSUM=1" : baseUrl;
    };
