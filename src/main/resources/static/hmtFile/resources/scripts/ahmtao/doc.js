$ahmtao.internal(function($ax) {
    ...
    var _initializePageFragment = function(pageFragment, objIdToObject) {
        ...
        var diagramObjectHelper = function(diagramObject, parent) {
            ...
            if(diagramObject.diagrams) { // 注释加密 
    ...
    (function() {
        ...
        var _getScriptIdFromPath = function(path, relativeTo, includeLimbo) {
            ...
            if(relativeTo) {
                ...
                } else if(typeof relativeTo === 'string') { // 注释加密 
                ...
                } else if(relativeTo instanceof Array) { // 注释加密 
    ...
    */
    $ax.public.navigate = $ax.navigate = function(to) { // 注释加密 
        var targetUrl;
        if(typeof (to) === 'object') {
            ...
            if(to.target == "new") {
                ...
                if (!_needsReload(targetLocation, to.url)) {
                    ...
                    targetLocation.href = $ahmtao.utils.getReloadPath() + "#" + encodeURI(targetUrl);
                }
            }
        } else {
            $ax.navigate({
    ...
    */
    $ax.public.reload = $ax.reload = function(includeVariables) {
        var targetUrl = (includeVariables === false)
            ? $ahmtao.utils.getReloadPath() + "#" + encodeURI($ax.pageData.url)
            : $ahmtao.utils.getReloadPath() + "#" + encodeURI($ax.globalVariableProvider.getLinkUrl($ax.pageData.url));
