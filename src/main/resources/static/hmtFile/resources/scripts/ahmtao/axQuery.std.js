...
$ahmtao.internal(function($ax) {
    ...
    $ax.constants.BUTTON_TYPE = 'button'; // 注释加密 
    ...
    $ax.public.fn.hide = function(options) {
        ...
        for(var index = 0; index < elementIds.length; index++) {
            var elementId = elementIds[index]; // 注释加密 
                ...
                children.each(function (i, child) {
                    ...
                    if($(child).css('position') === 'absolute') {
                        if(child.offsetLeft) {
                            ...
                            childCss.left = oldWidth ? child.offsetLeft * newLocationAndSizeCss.width / oldWidth : child.offsetLeft; // 注释加密 
                        }
                        if(child.offsetTop) {
                            childSizingObj.top = child.offsetTop;
                            childCss.top = oldHeight ? child.offsetTop * newLocationAndSizeCss.height / oldHeight : child.offsetTop; // 注释加密 
                        }
                    }
    ...
    $ax.public.fn.text = function() {
        if(arguments[0] == undefined) {
            ...
            for(var index = 0; index < elementIds.length; index++) {
                ...
                if($ax.public.fn.IsTextBox(widgetType) || $ax.public.fn.IsTextArea(widgetType)) { // 注释加密 
    ...
    $ax.public.fn.value = function() {
        if(arguments[0] == undefined) {
            ...
            if ($ax.public.fn.IsComboBox(widgetType) || $ax.public.fn.IsListBox(widgetType)) { // 注释加密 
                return $('#' + firstId + ' :selected').text();
            } else if ($ax.public.fn.IsCheckBox(widgetType) || $ax.public.fn.IsRadioButton(widgetType)) { // 注释加密 
                return $('#' + firstId + '_input').is(':selected');
            } else if ($ax.public.fn.IsTextBox(widgetType)) { // 注释加密 
            ...
            for(var index = 0; index < elementIds.length; index++) {
                ...
                if ($ax.public.fn.IsCheckBox(widgetType) || $ax.public.fn.IsRadioButton(widgetType)) { // 注释加密 
                    if(arguments[0] == true) {
            ...
            for(var index = 0; index < elementIds.length; index++) {
                ...
                if ($ax.public.fn.IsDynamicPanel(widgetType) || $ax.public.fn.IsLayer(widgetType)) {
                    ...
                    for(var i = 0; i < children.length; i++) {
                        $ahmtao('#' + children[i]).enabled(enabled);
                    }
                }
                var obj = $obj(elementId);
    ...
    $ax.public.fn.selected = function() {
        ...
        var enabled = arguments[0]; // 注释加密 

        for(var index = 0; index < elementIds.length; index++) {
            var elementId = elementIds[index];
            if(func) {
                enabled = func($ahmtao('#' + elementId));
            }

            var widgetType = $ax.getTypeFromElementId(elementId);

            if ($ax.public.fn.IsTreeNodeObject(widgetType)) { // 注释加密 
    ...
    var _bodyToWorld = $ahmtao.fn.bodyToWorld = function(x, from) {
        var body = $('body');
