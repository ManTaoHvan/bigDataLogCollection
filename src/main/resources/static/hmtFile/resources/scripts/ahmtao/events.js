...
$ahmtao.internal(function ($ax) {
    ...
    // 注释加密 
    $ahmtao.messageCenter.addStateListener("isTracing", function (key, value) {
    ...
    var ahmtao_TO_JQUERY_EVENT_NAMES = {
    ...
    // 注释加密 
    if(!$ax.features.supports.mobile) {
        ahmtao_TO_JQUERY_EVENT_NAMES.onClick = 'click';
        ahmtao_TO_JQUERY_EVENT_NAMES.onDoubleClick = 'dblclick';
        ahmtao_TO_JQUERY_EVENT_NAMES.onMouseMove = 'mousemove';
    }

    ahmtao_TO_JQUERY_EVENT_NAMES.onMouseDown = $ax.features.eventNames.mouseDownName;
    ahmtao_TO_JQUERY_EVENT_NAMES.onMouseUp = $ax.features.eventNames.mouseUpName;
    // 注释加密 
    var mouseEnterGuard = {};
    var _attachEvents = function (diagramObject, elementId, doMouseEnterGuard) {
        ...
        for(var eventName in diagramObject.interactionMap) {
            var jQueryEventName = ahmtao_TO_JQUERY_EVENT_NAMES[eventName];
    ...
    var _suppressedEvents = {}; // 注释加密 
    var _blockedEvents = {}; // 注释加密 
    ...
    var _handleEvent = $ax.event.handleEvent = function (elementId, eventInfo, axEventObject, skipShowDescriptions, synthetic) {
        ...
        if(_enteredWidgets[elementId] && eventDescription == 'OnMouseEnter') return; // 注释加密 
    ...
    var initSuppressingEvents = function () {
        ...
        for(var key in cancelSuppressions) {
            var jEventName = ahmtao_TO_JQUERY_EVENT_NAMES[key];
            ...
            if(($ax.public.fn.IsTextArea(dObj.type) || $ax.public.fn.IsTextBox(dObj.type)) && hasPlaceholder) {
                ...
                if(ANDROID) {
                    ...
                    inputJobj.bind('input', function() {
                        if(!dObj.HideHintOnFocused) { // 注释加密 
                            var id = this.id;
                            var inputIndex = id.indexOf('_input');
                            if(inputIndex == -1) return;
                            var inputId = id.substring(0, inputIndex);
    ...
    $ahmtao.initializeObjectEvents = function (query, refreshType, _) {
        // 注释加密 
    ...
    var _attachIxStyleEvents = function(dObj, elementId, $element, ignoreHasIxStyles) {
        ...
        var needsMouseFilter = (ignoreHasIxStyles || $ax.event.HasIxStyles(dObj))
            ...
            !$ax.public.fn.IsRepeater(dObj.type) // 注释加密 
        ...
        _event.initMobileEvents = function($tapQuery, $moveQuery, elementId) {
            ...
            $tapQuery.bind('touchstart', function(e) {
                ...
                if(time - lastClickEventTime < DBLCLICK_THRESHOLD_MS) {
                    ...
                    if(axEventObject) e.preventDefault(); // 注释加密 
                }
                ...
                if((xDiff * xDiff + yDiff * yDiff) < CLICK_THRESHOLD_PX_SQ) {
                    ...
                    if(time - lastClickEventTime < DBLCLICK_THRESHOLD_MS) {
                        ...
                        if(e.originalEvent && e.originalEvent.handled) e.preventDefault(); // 注释加密 
                    }
    ...
    var _postAdaptiveViewChanged = function(forceSwitchTo) {
        ...
        try {
            ...
            (!CHROME_5_LOCAL && window.parent.$ && window.parent.$('#mainFrame').length > 0)) {
                ...
                $ahmtao.messageCenter.postMessage('adaptiveViewChange', data);
            }
        } catch(e) { }
    };
    ...
    var fireEventThroughContainers = function(eventName, objects, synthetic, searchFilter, callFilter, path, itemId) {
        ...
        for(var i = 0; i < objects.length; i++) {
            ...
            if (!$ax.public.fn.IsReferenceDiagramObject(obj.type) && !$ax.public.fn.IsDynamicPanel(obj.type) && !$ax.public.fn.IsRepeater(obj.type) && !$ax.public.fn.IsLayer(obj.type)) {
                if(_callFilterCheck(callFilter)) { // 注释加密 
    ...
    var _pageLoad = function() {
        ...
        // 注释加密 
        var PAGE_ahmtao_TO_JQUERY_EVENT_NAMES = {
        ...
        var $win = $(window);
        if(!$ax.features.supports.mobile) {
            PAGE_ahmtao_TO_JQUERY_EVENT_NAMES.onClick = ['html', 'click'];
            PAGE_ahmtao_TO_JQUERY_EVENT_NAMES.onDoubleClick = ['html', 'dblclick'];
            PAGE_ahmtao_TO_JQUERY_EVENT_NAMES.onMouseMove = ['html', 'mousemove'];
        } else {
        ...
        for(key in PAGE_ahmtao_TO_JQUERY_EVENT_NAMES) {
            if(!PAGE_ahmtao_TO_JQUERY_EVENT_NAMES.hasOwnProperty(key)) continue;
            (function(ahmtaoName) {
                var jqueryEventNamePair = PAGE_ahmtao_TO_JQUERY_EVENT_NAMES[ahmtaoName];
                ...
                $(jObj)[actionName](function (e) {
                    $ax.setjBrowserEvent(e);
                    return fireEventThroughContainers(ahmtaoName, undefined, false, [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE, $ax.constants.DYNAMIC_PANEL_TYPE, $ax.constants.REPEATER],
                        [$ax.constants.PAGE_TYPE, $ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE]);
                });
            })(key);
        }
