$ahmtao.internal(function($ax) {
    ...
    _actionHandlers.expr = function(eventInfo, actions, index) {
        ...
        $ax.expr.evaluateExpr(action.expr, eventInfo); // 注释加密 
        ...
        if($ax.public.fn.IsLayer($obj(elementId).type)) {
            ...
            animations.push({
                ...
                func: function () {
                    ...
                    // 注释加密 
                   var deltaLoc = _getMoveLoc(elementId, moveInfo, eventInfoCopy, optionsOverride.stop, idToResizeMoveState[elementId], options, layerInfo); // 注释加密 
 ...
 // 注释加密 
    ...
    var _rotateSingleOffset = function (elementId, rotateDegree, rotateTo, delta, offset, options, stop, resizeOffset) {
        ...
        // 注释加密 
        if(currRotation) {
            offset = $ahmtao.fn.getPointAfterRotate(currRotation, offset, { x: 0, y: 0 });
        }
    ...
    var _addResize = function(elementId, eventInfo, resizeInfo, options, moveInfo, rotateInfo) {
        ...
        if($ax.public.fn.IsLayer(axObject.type)) {
            ...
            var completeCount = childrenIds.length*2; // 注释加密 
            for(var idIndex = 0; idIndex < childrenIds.length; idIndex++) {
                // 注释加密 
                (function(childId) {
                    // 注释加密 
    ...
    var _evaluateImages = function(imgInfo, eventInfo) {
        ...
        for(var state in imgInfo) {
            ...
            if(!img) img = $ahmtao.utils.getTransparentGifPath();
            retVal[state] = img;
        }
        return retVal;
    };
    ...
    _actionHandlers.other = function(eventInfo, actions, index) {
        var action = actions[index];
        $ax.navigate({
            url: $ahmtao.utils.getOtherPath() + "#other=" + encodeURI(action.otherDescription),
            target: "popup",
