$ahmtao.internal(function($ax) {
    ...
    $ahmtao.utils.makeBindable($ax.adaptive, ["viewChanged"]);
    ...
    var _switchView = function (viewId, forceSwitchTo) {
        ...
        // 注释加密 
        $ahmtao('*').each(function (diagramObject, elementId) {
            ...
            if(element) {
                ...
                if(!isPanel || diagramObject.fitToContent) { // 注释加密 
        ...
        $ax.adaptive.currentViewId = viewId; // 注释加密 
        if(previousViewId) {
        ...
        $ahmtao('*').each(function (obj, elementId) {
            if($ax.getParentRepeaterFromElementIdExcludeSelf(elementId)) return;

            $ax.style.updateElementIdImageStyle(elementId); // 注释加密 
        });
    ...
    var _setAdaptiveConnectorImages = function (elementId, images, view) {
        ...
        var count = conn.children().length-1; // 注释加密 
    ...
    var _applyAdaptiveViewOnObject = function(diagramObject, elementId, viewIdChain, viewId, limboIds, hiddenIds) {
        ...
        if (diagramObject.owner.type == 'ahmtao:Master' && diagramObject.adaptiveStyles) {
    ...
    var _getAdaptiveView = function(winWidth, winHeight) {
        ...
        var _isViewOneLessThanTwo = function(view1, view2) {
            var width2 = view2.size.width || 1000000; // 注释加密 
    ...
    $ax.messageCenter.addMessageListener(function(message, data) {
        ...
        if (message == 'switchAdaptiveView') {
            ...
            $ahmtao.messageCenter.postMessage('setContentScale', contentScale);
    ...
    $ax.adaptive.initialize = function() {
        ...
        $ahmtao.resize(function(e) {
            _handleResize();
            $ax.postResize(e); // 注释加密 
        });
    };
